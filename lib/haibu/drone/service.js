...
exports.createRouter = function (drone) {
  ...
  haibu.router.param(':userid', /([\w\-\.]+)/);
  haibu.router.param(':appid', /([\w\-\.]+)/);
  haibu.router.param(':id', /([\w\-\.]+)/);
  ...
  haibu.router.path('/apps', function () {
    ...
    this.get(function () {
      var res = this.res,
          data = { message: 'No user specified' };
    ...
    this.get('/:userid', function (userId) {
      drone.apps();
      //haibu.sendResponse(this.res, 200, drone.apps());
    });
  });  
  ...
  haibu.router.path('/drones', function () {
    ...
    // drone processes.
    //
    this.get('/running/:user', function (user) {
      haibu.sendResponse(this.res, 200, drone.running(user));
    });    
    ...
    this.post('/:id/stop', function (id) {
     var res = this.res
      , appId = this.req.body ? this.req.body.start: id;
      
      drone.stop(appId, function (err, result) {
        if (err || !result) {
    ...
    this.post('/:id/restart', function (id) {
      ...
      drone.restart(appId, function (err, drones) {
        if (err) {
          haibu.emit(['error', 'service'], 'error', err);
          return haibu.sendResponse(res, 500, { error: err });
        }
