  ...
  haibu.common.each(this.list(), function (record) {
    ...
    if(user) {
      haibu.common.each(record.drones, function (drone) {
        if (self.formatUser(app, drone, user))
          all.push(self.formatUser(app, drone, user));
      });
    } else {
      haibu.common.each(record.drones, function (drone) {
...
Drone.prototype.formatUser = function (app, drone, user) {
  var hosts = drone.host.split(' '),
      host2 = hosts.pop(),
      host1 = hosts.pop();
